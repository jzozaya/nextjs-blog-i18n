...
const Share = ({ title, description, slug, className }: ShareProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center pt-4 m-4 mt-8 sm:flex-row">
      <div className="mb-4 sm:mb-0">
        <p className="px-4 mr-3 font-bold text-highlighted dark:text-darkmode-highlighted text-primary-500 sm:border-r-2">
          {t('share')}
        </p>
      </div>
      <div>
        <ul className={`grid grid-cols-4 gap-4 ${className}`}>
          ...
          <li className="relative inline-block ml-4">
            <button
              onMouseEnter={() => setShowMenu(true)}
              onMouseLeave={() => setShowMenu(false)}
              onClick={handleCopy}
              className="text-gray-700 outline-none fill-current hover:text-primary-500 focus:outline-none dark:text-gray-200 dark:hover:text-primary-400"
            >
            ...
            {showMenu && (
              <div className="absolute right-0 w-32 p-2 text-center bg-white rounded-md shadow-lg top-8 ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800">
